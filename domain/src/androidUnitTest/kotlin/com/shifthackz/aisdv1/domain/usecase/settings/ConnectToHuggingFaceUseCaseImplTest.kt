...
class DefaultConnectToHuggingFaceUseCaseImplTest {
    ...
    private val stubTestHuggingFaceApiKeyUseCase = mockk<TestHuggingFaceApiKeyUseCase>()

    private val useCase = DefaultConnectToHuggingFaceUseCaseImpl(
        getConfigurationUseCase = stubGetConfigurationUseCase,
        setServerConfigurationUseCase = stubSetServerConfigurationUseCase,
        testHuggingFaceApiKeyUseCase = stubTestHuggingFaceApiKeyUseCase,
    )
    ...
    fun `given connection process successful, API key is valid, expected success result value`() = runTest {
        ...
        coEvery {
            stubTestHuggingFaceApiKeyUseCase()
        } returns true

        assertEquals(Result.success(Unit), useCase("5598", "5598"))
    }
