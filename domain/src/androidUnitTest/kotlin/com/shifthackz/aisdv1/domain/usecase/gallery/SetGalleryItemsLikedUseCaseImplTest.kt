...
class SetGalleryItemsLikedUseCaseImplTest {

    private val stubRepository = mockk<GenerationResultRepository>()

    private val useCase = SetGalleryItemsLikedUseCaseImpl(stubRepository)

    @Test
    fun `given repository updated data successfully, expected complete`() = runTest {
        coEvery {
            stubRepository.setLikedByIds(any(), any())
        } returns Unit

        useCase(listOf(5598L, 151297L), true)

        coVerify {
            stubRepository.setLikedByIds(listOf(5598L, 151297L), true)
        }
    }
