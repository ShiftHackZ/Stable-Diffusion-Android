...
class KtorSwarmUiLorasRemoteDataSourceTest {

    private val stubException = Throwable("Something went wrong.")
    private val stubApi = mockk<SwarmUiModelsApi>()

    private val remoteDataSource = KtorSwarmUiLorasRemoteDataSource(stubApi)
    ...
    fun `given attempt to fetch loras, api returns success response, expected valid loras list value`() = runTest {
        ...
        val actual = remoteDataSource.fetchLoras(BASE_URL, SESSION_ID, AuthorizationCredentials.None)
    ...
    fun `given attempt to fetch loras, api returns error response, expected error value`() = runTest {
        ...
        val actual = runCatching {
            remoteDataSource.fetchLoras(BASE_URL, SESSION_ID, AuthorizationCredentials.None)
        }
